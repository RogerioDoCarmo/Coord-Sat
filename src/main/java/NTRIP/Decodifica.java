...
public void getMessage(char c ) {
   ...
   public void decodifica(int cmsg) {
    ...
    switch(type) {
        /*------------------Estas mensagens não são utilizadas -------------------*/
    /*  case 1005 :case 1006:{
        System.out.println("\n\nMessage Number: " +nmsg+" - Estacao:"+mp);
        long dada =0;
        dada = (int)getBITS(dada,12);  //id
        SKIPBITS(10);

        double in=(double)getBITSSIGN(dada, 38);
        handle.antX = in/10000;
        System.out.println("X : "+handle.antX);

        SKIPBITS(2);

        in=(double)getBITSSIGN(dada, 38);
        handle.antY = in/10000;
        System.out.println("Y : "+handle.antY);

        SKIPBITS(2);

        in=(double)getBITSSIGN(dada, 38);
        handle.antZ= in/10000;
        System.out.println("Z : "+handle.antZ);

        if(nmsg == 1006) {
            dada = getBITS(dada, 16);
            handle.antH=dada;
            System.out.println("H : "+handle.antH);
        }
      } //1005 e 1006
      break;
     case 1007: case 1008: case 1033:
      {
        System.out.println("\n\nMessage Number: " +nmsg+" - Estacao:");
        char[] antenna=null;
        int antnum=0;
       // for(int v=0;v<size;v++) {int glo= (int)dado[v];System.out.println("\n\ndado: " +dado[v]);System.out.println("\n\ng: " +glo);}
        SKIPBITS(12);
        antenna=getSTRING(antnum,antenna);
        handle.antenna = antenna; //   memcpy(handle.antenna, antenna, antnum);
   //     handle.antenna[20] = 0;
      ...
      case 1001: case 1002: case 1003: case 1004: {
         System.out.println("\n\nMessage Number: " +nmsg+" - Estacao:");
          ...
          int[] lastlockl1 = new int[64];
          int[] lastlockl2 = new int[64];

          int numsats = 0, wasamb=0;

          for(i = 0; i < 64; ++i)
          lastlockl1[i] = lastlockl2[i] = 0;


          refs= getBITS(refs,12); //12 bits da referencia

	  data=getBITS(data,30); //30 bits TOW

           if(data/1000 < ((int)handle.GPSTOW - 86400)) ++handle.GPSWeek;
           handle.GPSTOW = data/1000;
           if(handle.DataNew.week != 0 && (handle.DataNew.timeofweek != data || handle.DataNew.week!= handle.GPSWeek))
           {
              handle.Data = handle.DataNew;
              handle.DataNew = new gnssdata();//  memset(gnss, 0, sizeof(*gnss));
              old = 1;
           }
           handle.DataNew.timeofweek=data;
           handle.DataNew.week=handle.GPSWeek;
        
           syncf=(int)getBITS(data,1); //1 BIT GNSS FLAG
           
           numsats=(int)getBITS(numsats,5); //5 bits numero de satelites
           System.out.println("No. of GPS Satellite Signals Processed:"+numsats);

           data=getBITS(data,1); //1 bit

           data=getBITS(data,3);//3 bits

           while(numsats-- != 0) { //enquanto existir satelites

                    int sv, c,l,s,ce,le,se;
                    long l1range, code,amb=0;
                    int fullsat, num;

                      //mensagens 1001
                    sv =(int)getBITS(data,6);//6 bits de id de satelite
                    fullsat = sv < 40 ? sv : sv+80;
                    for(num = 0; num < handle.DataNew.numsats && fullsat != handle.DataNew.satellites[num]; ++num);

                    if(num == handle.DataNew.numsats){ handle.DataNew.satellites[handle.DataNew.numsats++]= fullsat;}
 
                    code=(int)getBITS(data,1);//1 bit codigo

                    if(code !=0)
                     {
                       c = Constantes.GNSSDF_P1DATA;  ce = Constantes.GNSSENTRY_P1DATA;
                       l = Constantes.GNSSDF_L1PDATA; le = Constantes.GNSSENTRY_L1PDATA;
                       s = Constantes.GNSSDF_S1PDATA; se = Constantes.GNSSENTRY_S1PDATA;
                    }
                    else
                    {
                       c = Constantes.GNSSDF_C1DATA;  ce = Constantes.GNSSENTRY_C1DATA;
                       l = Constantes.GNSSDF_L1CDATA; le = Constantes.GNSSENTRY_L1CDATA;
                       s = Constantes.GNSSDF_S1CDATA; se = Constantes.GNSSENTRY_S1CDATA;
                    }

		    l1range=getBITS(data,24);//24 bits L1 Pseudorange

                    long db =getBITSSIGN(data, 20);

	            if((db & ((1<<20)-1)) != 0x80000)
                    {
                      handle.DataNew.dataflags[num] |= (c|l);
                      handle.DataNew.measdata[num][ce]= l1range*0.02;
                      handle.DataNew.measdata[num][le] = l1range*0.02 + db*0.0005;
                    }
                   
                    data=getBITS(data,7); //7 bits Lock

	            lastlockl1[sv] = (int)data;
                    if(handle.lastlockl1[sv] > (int)data)
                    handle.DataNew.dataflags[num] |= Constantes.GNSSDF_LOCKLOSSL1;

		    if(nmsg==1002||nmsg==1004) { //obtendo campos iguais na msg 1002 e 1004

                        amb=getBITS(data,8);//8 bits Lock Pseudorange Modulus Ambiguity
			
                        if(amb!= 0 && (handle.DataNew.dataflags[num] & c)!= 0)
                        {
                           handle.DataNew.measdata[num][ce] += amb*299792.458;
                           handle.DataNew.measdata[num][le] += amb*299792.458;
                           ++wasamb;
                        }

			data=getBITS(data,8);//8 bits CNR
			
                        if(data !=0 )
                        {
                           handle.DataNew.dataflags[num] |= s;
                           handle.DataNew.measdata[num][se] = data*0.25;
                           data /= 4*4;
                           if(data > 9) data = 9;
                           else if(data < 1) data = 1;
                           handle.DataNew.snrL1[num] = (int)data;
                        }

		    }
                    handle.DataNew.measdata[num][le] /= Constantes.GPS_WAVELENGTH_L1;
                    if(nmsg==1003||nmsg==1004) {//obtendo campos iguais na msg 1003 e 1004
			code=getBITS(data,2);

			if(code!= 0)
                        {
                            c = Constantes.GNSSDF_P2DATA;  ce = Constantes.GNSSENTRY_P2DATA;
                            l = Constantes.GNSSDF_L2PDATA; le = Constantes.GNSSENTRY_L2PDATA;
                            s = Constantes.GNSSDF_S2PDATA; se = Constantes.GNSSENTRY_S2PDATA;
                        }
                        else
                        {
                             c = Constantes.GNSSDF_C2DATA;  ce = Constantes.GNSSENTRY_C2DATA;
                             l = Constantes.GNSSDF_L2CDATA; le = Constantes.GNSSENTRY_L2CDATA;
                             s = Constantes.GNSSDF_S2CDATA; se = Constantes.GNSSENTRY_S2CDATA;
                        }
		        data=getBITSSIGN(data,14);
			
                        if((data &((1<<14)-1)) != 0x2000)
                        {
                             handle.DataNew.dataflags[num] |= c;
                             handle.DataNew.measdata[num][ce] = l1range*0.02 + data*0.02 + amb*299792.458;
                        }

			data=getBITSSIGN(data,20);

			if((data&((1<<20)-1)) != 0x80000)
                        {
                             handle.DataNew.dataflags[num] |= l;
                             handle.DataNew.measdata[num][le] = l1range*0.02+data*0.0005 +amb*299792.458;
                        }

                        data=getBITS(data,7);

		        lastlockl2[sv] = (int)data;
                        if(handle.lastlockl2[sv] > data)
                        handle.DataNew.dataflags[num] |= Constantes.GNSSDF_LOCKLOSSL2;



		    if(nmsg==1004) { //campo adicional na msg 1004
			data=getBITS(data,8);
			dbdata=data*0.25;
                        ...
                        if(data !=0 )
                        {
                            ...
                            handle.DataNew.snrL2[num] = (int)data;
                        }
		    }
                    handle.DataNew.measdata[num][le] /= Constantes.GPS_WAVELENGTH_L2;
           ...
           if (!(syncf != 0) || old != 0) {
                    if (wasamb != 0) { /* not RINEX compatible without */
                        ret = 1;
                    } else {
                        ret = 2;
                    }
        ...
        case 1009: case 1010: case 1011: case 1012: {
          System.out.println("\n\nMessage Number: " +nmsg+" - Estacao: PPTE1");
	  ...
	  refs =getBITS(refs,12); //id

	  data=getBITS(data,27);
         System.out.println("\n\ndata: " +data);

          Auxiliar aux = new Auxiliar(handle.GPSWeek,handle.GPSTOW);
          Tempo.updatetime(aux,(int)data,0);

          handle.GPSWeek = aux.week;
          handle.GPSTOW = aux.secOfWeek;

          data = handle.GPSTOW*1000;
          ...
          syncf=(int)getBITS(data,1);

          numsats=(int)getBITS(numsats,5);
	  System.out.println("No. of GLONASS Satellite Signals Processed: "+numsats);
          ...
          while(numsats-- != 0) {
	       ...
	       sv=(int)getBITS(data,6);

		fullsat = sv-1 + Constantes.PRN_GLONASS_START;
                for(num = 0; num < handle.DataNew.numsats && fullsat != handle.DataNew.satellites[num]; ++num);

                if(num == handle.DataNew.numsats)
                    handle.DataNew.satellites[handle.DataNew.numsats++] = fullsat;

		code=getBITS(data,1);

                freq=(int)getBITS(data,5);
		...
		if((data & ((1<<20)-1)) != 0x80000)
                {
                  ...
                  handle.DataNew.dataflags[num] |= (c|l);
                  handle.DataNew.measdata[num][ce] = l1range*0.02;
                  handle.DataNew.measdata[num][le] = l1range*0.02+data*0.0005;
                }
		...
		if(nmsg==1010||nmsg==1012) {
			amb=getBITS(data,7);
	
		        if(amb!= 0 && (handle.DataNew.dataflags[num] & c)!= 0)
                        {
                           handle.DataNew.measdata[num][ce] += amb*599584.916;
                           handle.DataNew.measdata[num][le] += amb*599584.916;
                           ++wasamb;
                        }
			...
			if(data!= 0)
                 ...
                 handle.DataNew.measdata[num][le] /= Constantes.GLO_WAVELENGTH_L1(freq - 7);

		if(nmsg==1011||nmsg==1012) {
			code=getBITS(data,2);
			...
			data=getBITS(data,14);
			
			 if((data & ((1<<14)-1)) != 0x2000)
                         {
                           handle.DataNew.dataflags[num] |= c;
                           handle.DataNew.measdata[num][ce] = l1range*0.02+data*0.02+amb*599584.916;
                         }
			...
			lastlockl2[sv] = (int)data;
		        ...
		        if(nmsg==1012) {
		          data=getBITS(data,8);
                        ...
                        handle.DataNew.measdata[num][le] /= Constantes.GLO_WAVELENGTH_L2(freq-7);
		 System.out.println("\n");
                 }
                 if(!(sv != 0) || sv > 24)
                      --handle.DataNew.numsats;
          }//fim while
          System.out.println("\n\n\n");
          for(i = 0; i < 64; ++i)
          {
